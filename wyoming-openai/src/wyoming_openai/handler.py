...
from .preprocessing import preprocess_for_tts, DEFAULT_REPLACEMENTS
...
class OpenAIEventHandler(AsyncEventHandler):
    ...
    def _preprocess_tts_text(self, text: str) -> str:
        """
        Preprocess text before sending to TTS.
        
        Removes emojis, markdown, applies custom replacements, etc.
        Can be disabled via tts_preprocessing_enabled.
        
        Args:
            text: Raw text from user/agent
            
        Returns:
            Cleaned text optimized for TTS
        """
        if not self._tts_preprocessing_enabled:
            return text
        ...
        return preprocess_for_tts(
            ...
            remove_emoji=True,
            remove_md=True,
            custom_replacements=self._tts_custom_replacements,
            normalize_pause=True
        )
    ...
    async def _handle_synthesize_chunk(self, synthesize_chunk: SynthesizeChunk) -> bool:
        ...
        self._text_accumulator += cleaned_chunk
    ...
    async def _stream_audio_to_wyoming(self, audio_data: bytes, is_first_chunk: bool, start_timestamp: float) -> float | None:
        ...
        try:
            ...
            if wav_params:
                ...
                # Try IEEE Float conversion
                ieee_result = self._convert_ieee_float_wav_to_pcm(audio_data)
                if ieee_result:
                    audio_data, audio_rate, audio_channels, audio_width = ieee_result
                    _LOGGER.debug("Converted IEEE Float WAV to PCM: %d Hz, %d channels, %d bytes/sample",
                    ...
                    async for chunk in response.iter_bytes(chunk_size=TTS_CHUNK_SIZE):
                        if first_chunk is None:
                            ...
                            if wav_params:
                                ...
                                # Try IEEE Float conversion
                                ieee_result = self._convert_ieee_float_wav_to_pcm(chunk)
                                if ieee_result:
                                    audio_data, audio_rate, audio_channels, audio_width = ieee_result
                                    _LOGGER.debug("Converted IEEE Float WAV to PCM: %d Hz, %d channels, %d bytes/sample",
                                                audio_rate, audio_channels, audio_width)
    ...
    def _convert_ieee_float_wav_to_pcm(self, wav_data: bytes) -> tuple[bytes, int, int, int] | None:
        """
        Convert IEEE Float WAV (format 3) to PCM WAV (format 1).
        
        Returns (pcm_data, sample_rate, channels, sample_width) or None on error.
        """
        try:
            ...
            while offset < len(wav_data) - 8:
                ...
                if chunk_id == b'fmt ':
                    # Parse fmt chunk
                    audio_format = struct.unpack('<H', wav_data[offset+8:offset+10])[0]
                    channels = struct.unpack('<H', wav_data[offset+10:offset+12])[0]
                    sample_rate = struct.unpack('<I', wav_data[offset+12:offset+16])[0]
                    sample_width = struct.unpack('<H', wav_data[offset+22:offset+24])[0] // 8
                    
                    _LOGGER.debug("Detected WAV format: %d, rate: %d Hz, channels: %d, width: %d bytes",
                                 audio_format, sample_rate, channels, sample_width)
                    
                    if audio_format != 3:  # Not IEEE Float
                        _LOGGER.debug("Not IEEE Float WAV (format %d), skipping conversion", audio_format)
                        return None
                    
                    # IEEE Float detected - continue to find data chunk
                    break
                
                offset += 8 + chunk_size
                if chunk_size % 2:  # Pad byte
                    offset += 1
            else:
                return None
            ...
            while offset < len(wav_data) - 8:
                ...
                if chunk_id == b'data':
                    # Found audio data - convert Float32 to Int16 PCM
                    data_offset = offset + 8
                    float_data = wav_data[data_offset:data_offset+chunk_size]
                    ...
                    _LOGGER.info("Converted IEEE Float WAV to PCM: %d samples, %d Hz, %d channels",
                                len(pcm_array), sample_rate, channels)
                    
                    return (pcm_data, sample_rate, channels, 2)  # Int16 = 2 bytes
            ...
            _LOGGER.exception("Error converting IEEE Float WAV to PCM: %s", e)
            return None
