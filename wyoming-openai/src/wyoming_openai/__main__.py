...
async def main():
    ...
    parser.add_argument(
        "--tts-preprocessing-enabled",
        type=lambda x: x.lower() in ('true', '1', 'yes'),
        default=os.getenv("TTS_PREPROCESSING_ENABLED", "true").lower() in ('true', '1', 'yes'),
        help="Enable text preprocessing for TTS (removes emojis, markdown, etc.). Default: true"
    )
    parser.add_argument(
        "--tts-custom-replacements",
        type=str,
        default=os.getenv("TTS_CUSTOM_REPLACEMENTS"),
    ...
    if args.tts_custom_replacements:
        import json
        try:
            tts_custom_replacements = json.loads(args.tts_custom_replacements)
            _logger.info("Loaded %d custom TTS replacements", len(tts_custom_replacements))
        except json.JSONDecodeError as e:
            _logger.error("Failed to parse TTS_CUSTOM_REPLACEMENTS JSON: %s", e)
            tts_custom_replacements = None
